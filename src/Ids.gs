...
function idNext(entityKey) {
  return idNextBatch(entityKey, 1)[0];
}
 ...
 */
function idNextBatch(entityKey, n) {
  ...
  try {
    var current = utilToInt(dbGetConfigValue(def.counterKey, '0')) || 0;
    var ids = [];
    for (var i = 1; i <= n; i++) {
      ids.push(def.prefix + '-' + utilPadNumber(current + i, def.padding));
    }
    dbSetConfigValue(def.counterKey, current + n);
    return ids;
  } finally {
    lock.releaseLock();
  }
