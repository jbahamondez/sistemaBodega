...
function dbGetSheet(sheetKey) {
  var def = CONFIG.SHEETS[sheetKey];
  if (!def) throw new Error('Hoja desconocida: ' + sheetKey);
  var sheet = dbGetSpreadsheet().getSheetByName(def.name);
  if (!sheet) {
    throw new Error(
 ...
 * en los bordes.
 */
function dbReadAll(sheetKey) {
  var def = CONFIG.SHEETS[sheetKey];
  var sheet = dbGetSheet(sheetKey);
...
function dbFindById(sheetKey, idValue) {
  var def = CONFIG.SHEETS[sheetKey];
  return dbFindOne(sheetKey, function (row) {
    return row[def.idColumn] === utilTrim(idValue);
  });
}

/** Devuelve la primera fila que cumple el predicado, o null. */
function dbFindOne(sheetKey, predicate) {
  var rows = dbReadAll(sheetKey);
...
function dbFindWhere(sheetKey, predicate) {
  return dbReadAll(sheetKey).filter(predicate);
}
 ...
 */
function dbAppendRow(sheetKey, obj) {
  dbAppendRows(sheetKey, [obj]);
}

/** Agrega varias filas en una sola escritura (más eficiente que una a una). */
function dbAppendRows(sheetKey, objs) {
 ...
 */
function dbUpdateById(sheetKey, idValue, patch) {
  var def = CONFIG.SHEETS[sheetKey];
  var row = dbFindById(sheetKey, idValue);
  if (!row) {
    throw new Error(
      'No existe registro con ' + def.idColumn + ' = "' + idValue +
      '" en la hoja ' + def.name + '.');
  }
  dbUpdateRowByIndex(sheetKey, row._rowIndex, patch);
  return row._rowIndex;
}

/** Actualiza columnas de una fila concreta (fila real de la hoja, base 1). */
function dbUpdateRowByIndex(sheetKey, rowIndex, patch) {
  var def = CONFIG.SHEETS[sheetKey];
  var sheet = dbGetSheet(sheetKey);
...
function dbGetConfigValue(clave, defaultValue) {
  var row = dbFindById('CONFIGURACION', clave);
  return row ? row.valor : (defaultValue === undefined ? null : defaultValue);
}

/** Escribe (crea o actualiza) un valor en la hoja Configuracion. */
function dbSetConfigValue(clave, valor) {
  var row = dbFindById('CONFIGURACION', clave);
  if (row) {
    dbUpdateRowByIndex('CONFIGURACION', row._rowIndex, { valor: String(valor) });
  } else {
    dbAppendRow('CONFIGURACION', { clave: clave, valor: String(valor) });
  }
}
